...
/*****************************************************************/
{
    ...
    // create systematics directories
    m_systematicsMap[""] = 0; // no systematics
    m_systematicsList.push_back("");
    unsigned sysIndex = 1;
    for(const auto& sys : m_reader.systematics())
	{
        m_systematicsMap[sys] = sysIndex;
        m_systematicsList.push_back(sys);
		TDirectory* dirPtr = m_outputFile->mkdir(sys.c_str());
		if(!dirPtr) std::cout<<"ERROR: Cannot mkdir in output histo file\n";
        sysIndex++;
	}
    //gROOT->cd();
...
/*****************************************************************/
unsigned IAnalysis::systematicNumber(const std::string& name)
/*****************************************************************/
{
    const auto& sysItr = m_systematicsMap.find(name);
    if(sysItr==m_systematicsMap.end())
    {
        std::cout<<"WARNING: Systematic "<<name<<" has not been registered\n";
        return 0;
    }
    return sysItr->second;
}
