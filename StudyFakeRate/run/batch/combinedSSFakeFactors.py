...
def applySys(fakeFactorsMC, fakeFactorsData):
    fractionsFile = "/afs/cern.ch/user/j/jsauvan/workspace/Projects/Htautau_Run2/Studies/FakeRate/ComputeBackgroundFractions/results/backgroundFraction_SS_Iso_Medium_mvis_vs_mt.root"
    createShift('FractionSys', 'ShiftFractionSSW_Up'  , 'WUp', fractionsFile, fakeFactorsMC, fakeFactorsData)
    createShift('FractionSys', 'ShiftFractionSSW_Down', 'WDown', fractionsFile, fakeFactorsMC, fakeFactorsData)
    createShift('FractionSys', 'ShiftFractionSSQCD_Up'  , 'QCDUp', fractionsFile, fakeFactorsMC, fakeFactorsData)
    createShift('FractionSys', 'ShiftFractionSSQCD_Down', 'QCDDown', fractionsFile, fakeFactorsMC, fakeFactorsData)
    createShift('FractionSys', 'ShiftFractionSSTT_Up'  , 'TTUp', fractionsFile, fakeFactorsMC, fakeFactorsData)
    createShift('FractionSys', 'ShiftFractionSSTT_Down', 'TTDown', fractionsFile, fakeFactorsMC, fakeFactorsData)
...
def applyStat(fakeFactorsMC, fakeFactorsData):
    ...
    for statShift in statShiftsW:
        ...
        createShift('FractionStat', 'ShiftStatFractionSSW_{SHIFT}'.format(SHIFT=shift)  , 'WStat_{SHIFT}'.format(SHIFT=shift), fractionsFile, fakeFactorsMC, fakeFactorsData)
    for statShift in statShiftsQCD:
        shift = statShift.split('_QCDStat_')[-1]
        print shift
        createShift('FractionStat', 'ShiftStatFractionSSQCD_{SHIFT}'.format(SHIFT=shift)  , 'QCDStat_{SHIFT}'.format(SHIFT=shift), fractionsFile, fakeFactorsMC, fakeFactorsData)
