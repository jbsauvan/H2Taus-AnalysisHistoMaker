...
import glob
from Toys import addFluctuatedRawFakeFactors, addFluctuatedCombinedFakeFactors
import copy
...
for sample in samples:
    ...
    batch[-1].tree = "tree"
    batch[-1].outputDirectory = "/afs/cern.ch/work/j/jsauvan/Projects/Htautau_Run2/Histos/StudyFakeRate/MuTau/FakeFactorUncertaintiesFromToys/{SAMPLE}".format(SAMPLE=sample[Name])
    ...
    for fakeFactorsType in fakeFactorsTypes:
        ...
        selectedFakeFactors = copy.deepcopy(fakeFactorsData[fakeFactorsType])
        ## Add fluctuated fake factors
        if fakeFactorsType!='Combined' and fakeFactorsType!='HighMT':
            addFluctuatedRawFakeFactors(selectedFakeFactors)
        else:
            addFluctuatedCombinedFakeFactors(selectedFakeFactors)
        for fakeFactor in selectedFakeFactors.values():
            # Only apply combined fake factors, and don't apply shifted fake factors
            if fakeFactorsType!='Combined' or 'Shift' in fakeFactor.Name: continue
            systematics += fakeFactor.Name
            systematics += ":"
        for fakeFactor in selectedFakeFactors.values():
            # Don't apply systematic shifts
            if 'Shift' in fakeFactor.Name: continue
